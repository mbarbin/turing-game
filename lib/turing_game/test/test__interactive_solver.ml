...
let simulate_all_hypotheses ~decoder ?only_first_n () =
  ...
  List.iter hypotheses ~f:(fun hypothesis ->
    ...
    in
    List.iter steps ~f:(fun step -> print_s [%sexp (step : Interactive_solver.Step.t)]))
;;
...
let%expect_test "interactive solver simulation decoder 1" =
  ...
  [%expect
    ...
    (Info
     (Test_result
      ((code 211) (verifier 09) (condition (Has_digit_count (digit 3) (count 0)))
    ...
    (Info
     (Test_result
      ((code 211) (verifier 09) (condition (Has_digit_count (digit 3) (count 1)))
